...
fn compare_againts_previous_alignments() -> Result<()> {
    ...
    let alignments: Result<Vec<_>> = fs::read_dir("tests/fixtures/alignments").context("Reading alignments")?.map(|dir_entry| {
        let dir_entry = dir_entry.unwrap();
        Alignment::read_fasta(dir_entry.path())
    }).collect();
    let mut alignments = alignments?;

    let pattern_set = read_patterns_from_file("tests/fixtures/w-3_d-7.out").context("Reading patterns")?;

    for alignment in alignments.iter_mut() {
        align(&mut alignment.unaligned_data, &pattern_set, AlignProgress::Hide);
